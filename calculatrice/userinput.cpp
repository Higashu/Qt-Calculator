#include <iostream> //A retirer sur Qt une fois execute modifier inutile
#include <regex>    //A retirer sur Qt
//#include<QRegExp>
...
bool UserInput::isOpUnaire(std::string operateur)
{
    for (auto element : operateurUnaire)
    {
        if (element == operateur)
            return true;
    }
    return false;
}

bool UserInput::isOpBinaire(std::string operateur)
{
    for (auto element : operateurBinaire)
    {
        if (element == operateur)
            return true;
    }
...
bool UserInput::isOpPile(std::string operateur)
{
    for (auto element : operateurPile)
    {
        if (element == operateur)
            return true;
    }
...
bool UserInput::isOpTernaire(std::string operateur)
{
    for (auto element : operateurTernaire)
    {
        if (element == operateur)
            return true;
    }
...
//QString::fromStdString(content) pour transformer de std::string en QString (encodage utf-8)
void UserInput::loadExpression()
{
    ...
    try
    {
        ...
        for (auto it = objet.begin(); it != objet.end(); it++)
        {
            ...
            commandeInterpreteur(stripCommande(expression + " " + commande + " STO", false));
        }
        file.close();
    }
    catch (...)
...
//qs.toStdString() Pour convertir en std::string a partir de QString
    ...
    for (auto it = TABEXPR.cbegin(); it != TABEXPR.cend(); it++)
    {
        ...
        if (expr->getTarget() != nullptr)
        {
            ...
            if (expr->getTarget()->getType() == programme)
                objet["commande"] = expr->getTarget()->toString();
            else
                objet["commande"] = expr->getTarget()->rawInput();
            objet["nom"] = expr->toString();
...
const std::vector<std::string> UserInput::stripCommande(std::string commande, bool pileToSave)
{

    std::vector<std::string> userInputStripped;

    /*
    //Pour Qt sinon ça merde...
    if (commande.isEmpty()){
    ...
    */
    ...
    if (!std::regex_match(commande, std::regex("(^([\\S]+\\s?)+[\\S]$)|(^[\\S]+$)|(^.{0}$)")))
    ...
    for (size_t i = 0; i <= commande.size(); i++)
    {
        ...
        else
        {
            ...
            if (!isspace(commande[i]) && commande[i] != '\0')
                temp.push_back(commande[i]);
            //if (commande[i].isSpace() || i == commande.size()-1 ){
            if (isspace(commande[i]) || i == commande.size())
            {
    ...
    {
        if (!std::regex_match(element, std::regex("(^[0-9]+$)|(^[0-9]*[.][0-9]+$)|(^[A-Z]{1}[A-Z0-9]*$)|(^'[A-Z]{1}[A-Z0-9]*'$)|(^\\[(\\s[\\S]+)*\\s\\]$)")) && !isOpBinaire(element) && !isOpPile(element) && !isOpUnaire(element) && !isOpTernaire(element))
...
void UserInput::commandeInterpreteur(std::vector<std::string> userInputStripped, bool programmeSanityCheck)
{
    ...
    for (auto element : userInputStripped)
    {
        ...
        //Plus de if else if a cause de Qt, sinon c'est le bordel
        ...
        if (std::regex_match(element, std::regex("^[0-9]+$")))
        {
            ...
            if (!programmeSanityCheck)
                USERINPUT.handleLitterale(std::pair<int, int>(std::stoi(element), entiere));
        ...
        //if(regExp.exactMatch(element)){
        if (std::regex_match(element, std::regex("^[0-9]*[.][0-9]+$")))
        {

            if (!programmeSanityCheck)
                USERINPUT.handleLitterale(std::pair<double, int>(std::stod(element), reelle));
            //USERINPUT.handleLitterale(std::pair <int,int> (element.toDouble(),entiere));
        ...
        if (std::regex_match(element, std::regex("^[A-Z]{1}[A-Z0-9]*$")) && !isOpPile(element) && !isOpBinaire(element) && !isOpUnaire(element) && !isOpTernaire(element))
        {

            if (!programmeSanityCheck)
                USERINPUT.handleLitterale(std::pair<std::string, int>(element, atome));
            ...
            else
            {
                ...
                if (!TABEXPR.existe("'" + element + "'"))
                    PILE.setMessage("Attention : Un programme avec un atome qui ne correspond a aucune expression a ete insere : " + element + ".");
            }
        ...
        //regExp.setPattern("^'[A-Z]{1}[A-Z0-9]*'$");
        //if(regExp.exactMatch(element)){
        if (std::regex_match(element, std::regex("^'[A-Z]{1}[A-Z0-9]*'$")))
        {

            if (!programmeSanityCheck)
                USERINPUT.handleLitterale(std::pair<std::string, int>(element, expression));
        ...
        //if(regExp.exactMatch(element)){
        if (std::regex_match(element, std::regex("^\\[(\\s[\\S]+)*\\s\\]$")))
        {
            ...
            if (element.size() < 4)
                ...
                commandeInterpreteur(stripCommande(element.substr(2, element.size() - 4), false), true);

            if (!programmeSanityCheck)
                USERINPUT.handleLitterale(std::pair<std::string, int>(element, programme));
...
void UserInput::handleLitterale(std::pair<std::string, int> paire)
{
...
void UserInput::handleOperateurUnaire(std::string element)
{
...
void UserInput::handleOperateurBinaire(std::string element)
{
...
void UserInput::handleOperateurTernaire(std::string element)
{
...
void UserInput::handleOperateurPile(std::string element)
{
    try
    {
        if (element == "CLEAR")
...
void UserInput::execute()
{
    ...
    while (commande != "q" && commande != "Q")
    {
        ...
        PILE.manageMemory();
        PILE.afficher();
        std::cout << PILE.getMessage() << "\n";

        //Saisie utilisateur
        std::cout << "Votre commande > ";
        std::getline(std::cin, commande); //std::cin marche pas recupère juste 1 mot, guess how i know... :'(
    ...
    std::cout << "Ciaaooo, amigo de la noche !!";
}

/*
void UserInput::execute(std::string commande){
    try
    {
        USERINPUT.commandeInterpreteur(USERINPUT.stripCommande(commande));
        PILE.manageMemory();
