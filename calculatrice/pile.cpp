#include <iostream> //Normalement inutile pour l'affichage a retirer sur Qt par la suite
#include "pile.h"
...
std::string PileAffichage::WHILE(Litterale *lit1, Litterale *lit2)
{
...
Litterale *TableauExpression::getExpression(const std::string s) const
{
    ...
    throw(CompCeption("Erreur : Aucune expression ne correspond à l'atome " + s.substr(1, s.size() - 2) + "."));
}

bool TableauExpression::existe(std::string s) const
{
    for (auto element : tableauExpr)
    {
        if (element->toString() == s)
