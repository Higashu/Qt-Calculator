...
Litterale *LitteraleFactory::createLitterale(std::string s, typeLitterale t)
{
    if (t == expression)
...
void LitteraleManager::removeLitterale(Litterale *lit)
{
    for (size_t i = 0; i < listeLitterales.size(); i++)
    {
        if (listeLitterales[i] == lit)
        {
            ...
            listeLitterales.erase(listeLitterales.begin() + static_cast<int>(i));
            return;
        }
    }
}
...
const std::string LitteraleEntiere::rawInput() const {
    if (value < 0)
    {
        return std::to_string(-value)+" NEG";
    }
...
const std::string LitteraleReelle::rawInput() const {
    if (value < 0)
    {
        return std::to_string(-value)+" NEG";
    }
...
const std::string LitteraleRationnelle::rawInput() const {
    //Quand la fraction est negative, seul le numérateur est négatif
    if (numerateur < 0)
    {
        return std::to_string(-numerateur)+" "+std::to_string(denominateur)+" / NEG";
    }
