...
public:
    ...
    virtual const QString rawInput() const = 0;
...
public:
    const QString toString() const override { return QString::number(value); }
    const QString rawInput() const override;
...
public:
    const QString toString() const override { return QString::number(numerateur) + "/" + QString::number(denominateur); }
    const QString rawInput() const override;
...
private:
    LitteraleProgramme(QString prog) : Litterale(programme), program(prog){};
    ~LitteraleProgramme() = default;

public:
    const QString toString() const override { return program; }
    //const QString rawInput() const override {if (program.size() > 3) return program.mid(2,program.size()-4); else return "";}
    const QString rawInput() const override
    {
        if (program.size() > 3)
            return program.mid(2, program.size() - 4);
        else
            return "";
    }
    QString getProgramme() const { return program; }
    Litterale *getCopie() const override;
...
private:
    LitteraleExpression(QString expr) : Litterale(expression), express(expr){};
    ~LitteraleExpression() = default;

public:
    const QString toString() const override { return express; };
    const QString getExpression() const { return express; }
    const QString rawInput() const override { return express; }
    Litterale *getCopie() const override;
...
public:
    ...
    static Litterale *createLitterale(QString, typeLitterale);
